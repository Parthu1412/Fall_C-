...
if [ "${FALL_SKIP_BUILD:-0}" != "1" ]; then
    ...
    if [ "$need_configure" -eq 1 ]; then
        ...
        if [ -z "${TORCH_CUDA_ARCH_LIST:-}" ] && [ -z "${FALL_TORCH_CUDA_ARCH_LIST:-}" ]; then
        ...
        _EXTRA_CMAKE=()
        [ -n "${FALL_TORCH_CUDA_ARCH_LIST:-}" ] && _EXTRA_CMAKE+=("-DFALL_TORCH_CUDA_ARCH_LIST=${FALL_TORCH_CUDA_ARCH_LIST}")
...
elif [ -d "/opt/fall/vcpkg-libs" ];                   then _ssl_dir="/opt/fall/vcpkg-libs"
else                                                       _ssl_dir=""
fi
...
echo "Starting msg_gen (video creation service)..."
"$BUILD_DIR/msg_gen" &
PIDS["msg_gen"]=$!

echo "Starting fall_inference..."
