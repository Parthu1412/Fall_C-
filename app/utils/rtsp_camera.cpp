...
void RTSPCamera::readFramesLoop()
{
    ...
    const auto frame_interval =
        std::chrono::microseconds(static_cast<long long>(1e6 / std::max(1, target_fps_)));
    ...
    while (running_)
    {
        ...
        try
        {
            ...
            if (!ret || frame.empty())
            {
                ...
                {
                    Logger::warning(
                        "[RTSPCamera] RTSP stream stalled, attempting reconnect"
                        " | url=" +
                        url_ + " | consecutive_failures=" + std::to_string(consecutive_failures_) +
            ...
            if (elapsed >= 1.0)
            {
                ...
                {
                    double received_fps =
                        log_elapsed > 0 ? frames_received_count_ / log_elapsed : 0.0;
                    double sampled_fps =
                        log_elapsed > 0 ? frames_sampled_count_ / log_elapsed : 0.0;
        ...
        } catch (const std::exception& e)
        {
            ...
            {
                Logger::error(
                    "[RTSPCamera] Error reading frame in background thread"
                    " | url=" +
                    url_ + " | error=" + e.what());
            }
