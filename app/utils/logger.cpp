...
void Logger::set_level_from_env() {
    ...
    if (!val) return;
    std::string s(val);
    if (s == "DEBUG") current_level_ = LogLevel::DEBUG;
    else if (s == "INFO") current_level_ = LogLevel::INFO;
    else if (s == "WARNING") current_level_ = LogLevel::WARNING;
    else if (s == "ERROR") current_level_ = LogLevel::ERROR;
}
