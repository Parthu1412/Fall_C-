...
public:
    ...
    void reconnect() {
        ...
        try {
            ...
            opts.host = config.redis_host;
            opts.port = config.redis_port;
            if (!config.redis_password.empty()) opts.password = config.redis_password;
            redis_ = std::make_unique<Redis>(opts);
            redis_->ping();
...
private:
    ...
    std::optional<int> get_latest_frame_number() {
        ...
        if (!val_opt) {
            app::utils::Logger::warning("[RedisConsumer] No latest frame data in Redis for camera: " + camera_id_);
            return std::nullopt;
        }
        try {
    ...
    bool is_behind(int frame_number) const {
        if (frame_number_ < frame_number) {
            app::utils::Logger::warning("[RedisConsumer] Consumer behind producer for " + camera_id_);
            return true;
        }
        return false;
    }
    ...
    std::optional<int> calculate_target_frame(int latest_frame_number) {
        if (fps_ <= 0) return frame_number_;
    ...
    bool should_wait_or_reset(int latest_frame_number) {
        if (is_ahead(latest_frame_number)) return true;
    ...
    ReadResult parse_frame_payload(const std::string& data) {
        ...
        try {
            ...
            if (out_len == 0) return r;
            r.frame = cv::imdecode(img_bytes, cv::IMREAD_COLOR);
            return r;
        } catch (const json::exception& e) {
            app::utils::Logger::warning("[RedisConsumer] JSON parse error: " + std::string(e.what()));
            return r;
        }
    }
    ...
    static size_t base64_decode(const std::string& in, std::vector<uchar>& out) {
        ...
        for (int i = 0; i < 64; ++i) T[static_cast<uchar>(tbl[i])] = i;
        ...
        for (size_t i = 0; i < len; ++i) {
            ...
            bits += 6;
            if (bits >= 0) {
                if (out_len < out.size()) out[out_len++] = static_cast<uchar>((val >> bits) & 0xFF);
                bits -= 8;
            }
        }
        out.resize(out_len);
