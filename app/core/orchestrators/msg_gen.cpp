...
static void process_task(app::utils::AwsApiManager& /*aws_life*/, app::utils::S3Client& s3,
                         ...
                         const app::core::orchestrators::ZmqVideoTaskPacket& ev)
{
    ...
    std::string video_path = tmp_dir + "/" + trace_id + "_" + std::to_string(camera_id) +
                             "_timestamp_" + tss.str() + ".mp4";
    ...
    {
        app::utils::Logger::warning("[MsgGen] Video encode failed, dropping task trace_id=" +
                                    trace_id);
        return;
    }
    ...
    {
        std::string queue =
            cfg.use_generic_queue ? cfg.generic_queue_name : ("fall_" + std::to_string(store_id));
...
int main()
{
    ...
    for (int a = 0; a < cfg.max_retries; ++a)
    {
        ...
        if (rmq->is_connected() || cfg.rabbitmq_host.empty())
            break;
        app::utils::Logger::error("[MsgGen] RabbitMQ connect failed attempt " +
                                  std::to_string(a + 1));
    ...
    if (!cfg.rabbitmq_host.empty() && rmq && !rmq->is_connected())
    {
        app::utils::Logger::error(
            "[MsgGen] RabbitMQ connection failed after max retries — exiting");
        return 1;
    }
    ...
    app::utils::Logger::info("Bound to port " + std::to_string(cfg.video_sender_port) +
                             " for receiving from fall orchestrator");
    ...
    while (!g_stop)
    {
        ...
        active_tasks.push_back(
            std::async(std::launch::async, [ev = std::move(ev), &aws_life, &s3, &kafka]() {
