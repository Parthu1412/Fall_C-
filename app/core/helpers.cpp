...
bool is_valid_coordinate(int x, int y, int frame_width, int frame_height) {
    ...
    bool is_at_corner = (x < corner_tolerance && y < corner_tolerance) ||                    
...
void draw_keypoints(cv::Mat& frame,
                    ...
                    int frame_height)
{
    ...
    for (const auto& conn : connections) {
        ...
        if (start >= 17 || end >= 17 || start >= static_cast<int>(keypoints.size()) || end >= static_cast<int>(keypoints.size())) {
            continue;
        }
