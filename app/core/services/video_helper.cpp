...
// Writes a video file from a sequence of OpenCV Mats, using FFmpeg for encoding with an OpenCV
// fallback.
bool VideoHelper::write_video(const std::vector<cv::Mat>& frames, const std::string& output_path,
                              int fps)
{
    if (frames.empty())
    {
    ...
    } catch (const std::filesystem::filesystem_error& e)
    {
        app::utils::Logger::warning(
            std::string("[VideoHelper] Could not create output directory: ") + e.what());
    }
    ...
    {
        app::utils::Logger::warning(
            "[VideoHelper] FFmpeg pipe failed to open. Falling back to OpenCV VideoWriter.");
        ...
        writer.release();
        app::utils::Logger::info("[VideoHelper] Video written successfully via OpenCV fallback: " +
                                 output_path);
        return true;
    }
    ...
    if (returnCode != 0)
    {
        ...
        {
            char buf[256];
            while (std::fgets(buf, sizeof(buf), ferr))
                ffmpeg_err += buf;
            std::fclose(ferr);
        }
        std::filesystem::remove(ffmpeg_log);
        app::utils::Logger::error("[VideoHelper] FFmpeg error for " + output_path + ": " +
                                  ffmpeg_err);
        return false;
    }
    std::filesystem::remove(ffmpeg_log);
...
// Overload: accepts pre-encoded JPEG bytes — no re-encode needed, just pipe to FFmpeg or fallback
// to OpenCV if pipe fails.
bool VideoHelper::write_video(const std::vector<std::vector<uchar>>& jpeg_frames,
                              const std::string& output_path, int fps)
{
    ...
    {
        app::utils::Logger::warning(
            "[VideoHelper] FFmpeg pipe failed. Falling back to OpenCV VideoWriter.");
        ...
        if (first.empty())
        {
            app::utils::Logger::error(
                "[VideoHelper] Could not decode any frame for OpenCV fallback.");
            return false;
        }
        // --- OpenCV Fallback Implementation ---
    ...
    int returnCode = PCLOSE(pipe);
    // In POSIX, pclose returns the exit status of the shell. A non-zero return code means FFmpeg
    // failed or crashed.
    if (returnCode != 0)
    {
        std::string ffmpeg_err;
