...
FallDetectionService::FallDetectionService(bool annotate_image) : annotate_image_(annotate_image)
{
    ...
    const std::vector<std::vector<std::vector<float>>>& redis_style_detections)
{
    ...
    auto& cfg = app::config::AppConfig::getInstance();
    // Run pose inference and evaluate keypoints to determine if a fall is detected. Annotate frame
    // if enabled.
    try
    {
        ...
        // Evaluate each detected person's keypoints for fall angles and determine if a fall is
        // detected based on thresholds. Log details if a fall is detected.
        for (const auto& keypoints : keypoints_list)
        {
            ...
            app::utils::Logger::info(
                format_fall_detected_log(eval.left_leg_deg, eval.right_leg_deg, eval.torso_deg));
        }
