...
public:
                                      ...
                                      const std::string& /*oauthbearer_config*/) override
    {
        ...
        const RdKafka::ErrorCode code =
            handle->oauthbearer_set_token(tok, 900000, "kafka-cluster", {}, errstr);
...
void KafkaProducer::produce(const std::string& topic, const app::utils::FallMessage& message)
{
    ...
    nlohmann::json j = {
        {"store_id", message.store_id}, {"moksa_camera_id", message.moksa_camera_id},
        {"s3_uri", message.s3_uri},     {"video_uri", message.video_uri},
        {"trace_id", message.trace_id}, {"timestamp", message.timestamp},
    };
