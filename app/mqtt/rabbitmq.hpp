...
public:
    ...
    void connect()
    {
        ...
        if (config.rabbitmq_host.empty())
        {
            app::utils::Logger::info(
                "[RabbitMQ] RABBITMQ_HOST not set – RabbitMQ publishing disabled.");
            return;
        }
            ...
            } catch (const std::exception& e)
            {
                ...
                if (retry_count < max_retries_)
                    std::this_thread::sleep_for(
                        std::chrono::seconds(5));  // Match Python retry_delay=5
            }
        }
    ...
    void publish(const std::string& queue_name, const app::utils::FallMessage& message)
    {
        ...
        } catch (const std::exception& e)
        {
            ...
            app::utils::Logger::warning(
                "[RabbitMQ] Publish failed (stale connection), reconnecting and retrying | error=" +
            ...
            app::utils::Logger::info("[RabbitMQ] Published after reconnect trace_id: " +
                                     message.trace_id + " to queue: " + queue_name);
        }
    }
